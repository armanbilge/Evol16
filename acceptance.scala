...
val (vuHMC, refHMC) = {
  ...
  (new PhyloHMC[Double, Int](post, M, 1.0, 0.0, 0, identity[Double]) with VuLeapProg[Double, Int], new PhyloHMC[Double, Int](post, M, 1.0, 0.0, 0, identity[Double]) with ReflectiveLeapProg[Double, Int])
}
val q = cg_ho.modifyLengths(_.updated(4, 0.125))
val p = IndexedSeq.fill(5)(0.0)
val z = Z(q, p, vuHMC.U, vuHMC.K)
val initH = z.H
val L = 1000
val eps = 1.0 / 512.0
((0 until L).scanLeft(z)((z, _) => vuHMC.leapprog(eps)(z)), (0 until L).scanLeft(z)((z, _) => refHMC.leapprog(eps)(z))).zipped.foreach { (z1, z2) =>
  val a1 = math.min(math.exp(initH - z1.H), 1)
  val a2 = math.min(math.exp(initH - z2.H), 1)
  println(Traversable[Any](a1, a2).mkString("\t"))
}
